...
// Check if user has booked this specific service (for display purposes only)
...
// Simplified - only verification needed to review
$can_review = $is_verified;
...
<div class="modal fade" id="bookingModal" tabindex="-1" aria-labelledby="bookingModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="../controllers/book_service.php" method="post">
                <div class="modal-body">
                    <input type="hidden" name="listing_id" value="<?php echo $listing_id; ?>">
                    <p>Are you sure you want to book "<?php echo htmlspecialchars($listing['Short_Desc']); ?>"?</p>

                    <?php if (isset($_SESSION['user_id']) && !$is_verified): ?>
                        <div class="alert alert-info">
                            By booking this service, your account will be verified and you'll be able to leave reviews.
                        </div>
                    <?php endif; ?>

                    <?php if ($price_tiers->num_rows > 0): ?>
                        <div class="mb-3">
                            <label class="form-label">Select Pricing Tier:</label>
                            <?php $price_tiers->data_seek(0); ?>
                            <?php while ($tier = $price_tiers->fetch_assoc()): ?>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name="tier_id"
                                        id="tier<?php echo $tier['TierID']; ?>"
                                        value="<?php echo $tier['TierID']; ?>"
                                        <?php if ($price_tiers->num_rows == 1) echo 'checked'; ?>>
                                    <label class="form-check-label" for="tier<?php echo $tier['TierID']; ?>">
                                    ...
                                    <?php if (!empty($tier['Description'])): ?>
                                        <button type="button" class="btn btn-sm btn-link text-decoration-none"
                                            ...
                                            title="<?php echo htmlspecialchars($tier['Description']); ?>">
    ...
    <div class="container mt-2">
        ...
        <div class="row mb-4">
            ...
            <div class="col-4 d-flex align-items-center">
                <a href="../all_listings.php?id=<?php echo $listing['UserID']; ?>" class="btn btn-outline-primary">
                    <i class="fas fa-store me-2"></i>View All Listings
                </a>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                <div class="mb-4 d-grid gap-2">
                    <button type="button" class="btn btn-primary py-2" data-bs-toggle="modal" data-bs-target="#bookingModal">
                        <i class="fas fa-calendar-check me-2"></i>Book Now
                    </button>
                    <a href="../test2.php?business_id=<?php echo $listing['UserID']; ?>&listing_id=<?php echo $listing_id; ?>"
        ...
        <div class="row mt-5">
            <div class="col-12">
                ...
                <?php if (isset($_SESSION['user_id'])): ?>
                    <div class="review-form mb-4 p-3 bg-light rounded <?php if (!$can_review) echo 'disabled-review'; ?>">
                        <h4>Leave a Review</h4>
                        <?php if (!$can_review): ?>
                            <div class="alert alert-warning">
                                You need to book this service before you can leave a review.
                            </div>
                        <?php endif; ?>
                        <form action="../controllers/submit_review.php" method="post">
                                ...
                                <div class="rating-input">
                                    <?php for ($i = 5; $i >= 1; $i--): ?>
                                        <input type="radio" id="star<?php echo $i; ?>" name="rating" value="<?php echo $i; ?>"
                                            <?php if (!$can_review) echo 'disabled'; ?> required>
                                        <label for="star<?php echo $i; ?>" onclick="event.preventDefault(); setRating(<?php echo $i; ?>);"><i class="far fa-star"></i></label>
                                    <?php endfor; ?>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="reviewText" name="review_text" rows="3"
                                    <?php if (!$can_review) echo 'disabled'; ?> required maxlength="500"></textarea>
                                <small class="text-muted">Maximum 500 characters</small>
                            </div>

                            <button type="submit" class="btn btn-primary" <?php if (!$can_review) echo 'disabled'; ?>>Submit Review</button>
                        </form>
                    </div>
                <?php else: ?>
                            ...
                            <div class="col-12 mb-4">
                                <div class="review border p-3 rounded h-100">
                                    <div class="d-flex align-items-center mb-2">
                                        ...
                                        <?php
                                        $is_customer = true; // Default to true since we're joining with Customers table
                                        $is_current_user = isset($_SESSION['user_id']) && $_SESSION['user_id'] == $review['CustomerID'];

                                        if ($is_customer && !$is_current_user): ?>
    ...
    <script>
        // Make star rating interactive without jumping to anchor
        function setRating(rating) {
            const ratingInput = document.querySelector(`input[name="rating"][value="${rating}"]`);
            if (ratingInput && !ratingInput.disabled) {
        ...
        // Refresh page after successful booking
        document.addEventListener('DOMContentLoaded', function() {
            const successAlert = document.querySelector('.alert-success');
            if (successAlert && successAlert.textContent.includes('booked successfully')) {
                window.location.reload();
            }
            
            // Initialize carousel with automatic cycling
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            carousel.addEventListener('slid.bs.carousel', function(event) {
                ...
                document.getElementById('imageCounter').textContent = `Image ${currentSlideIndex + 1} of ${totalThumbnails}`;
